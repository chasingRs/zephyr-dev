...
static struct pwm_dt_spec pwm_bk = PWM_DT_SPEC_GET(DT_ALIAS(pwm_led_bk));
...
int main(void) {
    ...
    if (!pwm_is_ready_dt(&pwm_bk)) {
        LOG_ERR("PWM device not ready");
        return -1;
    }
    ret = pwm_set_dt(&pwm_bk, PWM_MSEC(1),PWM_MSEC(1));
    if (ret != 0) {
        LOG_ERR("PWM device set failed");
        return -1;
    }
